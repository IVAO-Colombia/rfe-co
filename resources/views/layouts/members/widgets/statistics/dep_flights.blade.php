@php
    $dep_flights= DB::select('SELECT COUNT(`flight`) FROM `bookings` WHERE `type` = "departure"');
    foreach ($dep_flights as $flights) {
    }
    $total_flights = $flights->{'COUNT(`flight`)'};
    $bookings=DB::select('SELECT COUNT(`user_id`) FROM `bookings` WHERE `type` = "departure"');
